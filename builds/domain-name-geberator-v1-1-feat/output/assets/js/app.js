class DomainGenerator {
    ...
    displayResults(domains, tld) {
        ...
        domains.forEach((domainData, index) => {
            const domainName = typeof domainData === 'object' ? domainData.name : domainData;
            const rating = typeof domainData === 'object' ? domainData.rating : 5;
            const domainFull = `${domainName}.${tld}`;
            const item = this.createDomainItem(domainFull, index, rating);
            this.elements.resultsContainer.appendChild(item);
        });
    }

    createDomainItem(domain, index, rating = 5) {
        ...
        div.innerHTML = `
            <div class="domain-info">
                ...
                <span class="domain-rating" title="Marketability Rating">${rating}/10</span>
            </div>
    ...
    updateDomainStatuses(results) {
        results.forEach(result => {
            ...
            if (result.rating && !domainItem.dataset.rating) {
                const ratingElement = domainItem.querySelector('.domain-rating');
                if (ratingElement) {
                    ratingElement.textContent = `${result.rating}/10`;
                    domainItem.dataset.rating = result.rating;
                }
            }
