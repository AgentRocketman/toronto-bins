...
- **Direct agent turn marker (2026-07-07):** `🎙️ [Voice <req_id>] "<transcription>"` — fires via `/hooks/agent` to main session. Respond as Nexos (not as GetMyBin FAQ bot). Keep reply 1–2 sentences (TTS'd). POST reply to voice bridge AND output the same short reply in Telegram so the user has a reference of what was said.
